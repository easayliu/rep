...
time_emoji_symb = [emojize(":%s:" %s) for s in all_time_emoji_name]
    ...
    while True:
        try:
            ...
            if seco=='00' or seco=='30':
                ...
                if for_fun < 0.10:
                    last_name = '人有三省 %s:%s %s %s %s' % (hour, minu, p, abbwn, random_emoji)
                elif for_fun < 0.30:
                    last_name = '人有三省 %s:%s %s %s %s' % (hour, minu, p, abbwn, random_emoji)
                elif for_fun < 0.60:
                    last_name = '人有三省 %s:%s %s GMT %s' % (hour, minu, p, random_emoji)
                elif for_fun < 0.90:
                    last_name = '人有三省 %s:%s %s GMT %s' % (hour, minu, p, random_emoji)
                else:
                    last_name = '人有三省 %s:%s %s GMT %s' % (hour, minu, p, random_emoji)
